...
interface Booking {
    ...
    status: string;
    can_cancel: boolean;
    treatment_name?: string; // Added treatment name field
}
...
export default function MyBookings() {
    ...
    // Fetch bookings
    const fetchBookings = useCallback(async (pageNumber = 1, shouldRefresh = false) => {
        try {
            if (shouldRefresh) {
                setRefreshing(true);
    ...
    // Reschedule booking
    const handleRescheduleBooking = useCallback((bookingId: number) => {
        setModalVisible(false);
    ...
    const renderBookingItem = ({item}: { item: Booking }) => (
        <TouchableOpacity
            ...
            // onPress={() => router.push(`/BookingDetailsScreen?id=${item.id}`)}
            onPress={() => router.push(`/(screens)/BookingConfirmationScreen?bookingId=${item.id}`)}
        >
            {/* Service image and info */}
            <View className="flex-row">
                <View className="w-1/3" style={styles.imageContainer}>
                    <Image
                        source={item.service.image
                ...
                <View className="w-2/3 p-3">
                    ...
                    <View className="flex-row items-center mb-1">
                        ...
                        <Text className="text-xs text-gray-700 ml-1">{item.formatted_date}</Text>
                        <Text className="text-xs text-gray-700 mx-1">|</Text>
                        <Feather name="clock" size={12} color="#666"/>
                        <Text className="text-xs text-gray-700 ml-1">{item.formatted_time}</Text>
                    </View>
    ...
    const renderEmptyState = () => {
        ...
        return (
            <View className="flex-1 justify-center items-center py-10">
                ...
                <Text className="text-lg font-bold mt-4 text-center">No Bookings Yet</Text>
                <Text className="text-gray-500 text-center mt-2 px-10">
                    You don't have any bookings at the moment. Book a service to get started.
                </Text>
                <TouchableOpacity
    ...
    return (
        <SafeAreaProvider>
            <SafeAreaView className="flex-1 bg-[#FAFAFA]">
                ...
                {/*    <Text className="text-xl font-bold text-black">My Bookings</Text>*/}
                {/*    <Text className="text-gray-500">Manage your appointments</Text>*/}
                {/*</View>*/}
                                ...
                                {selectedBooking && (
                                    <>
                                        ...
                                        >
                                            ...
                                            <Text style={[styles.modalOptionText, {color: '#dc3545'}]}>Cancel Booking</Text>
                                        </TouchableOpacity>
                                    </>
                                )}
                            </View>
